  ...
  return (
    <div className="grid grid-cols-5 gap-5 text-lg font-bold p-8 max-[500px]:grid-cols-1 max-[500px]:gap-2 max-[800px]:grid-cols-2 max-[800px]:gap-2 max-[1200px]:grid-cols-3 max-[1200px]:gap-2 max-[1600px]:grid-cols-4 max-[1600px]:gap-2">
      ...
      {uniqueProductTypes
        ...
        .map((item, index) => (
          <div key={index}>
            <div className="cursor-pointer box-size-Images" onClick={() => SelectProduct(item)}>
              <div>
                <Image
                  loader={myLoader}
      ...
      {uniqueProductTypes.some((t) => t.split("|")[0] === "null") &&
        displayProducts.map((prod, pIdx) => (
          <div key={`prod-${pIdx}`}>
            <div className="cursor-pointer box-size-Images" onClick={() => SelectProductReferrence(prod)}>
              <Image
                loader={myLoader}
      ...
      {isLoadingRef && (
        <div className="fixed inset-0 z-[1002] bg-black/50 flex items-center justify-center backdrop-blur-sm">
          <div className="flex flex-col items-center">
            ...
            <p className="text-white mt-4 font-normal">กำลังโหลดข้อมูล Reference...</p>
          </div>
        </div>
      )}
      ...
      {showRefModal && (
        <div className="fixed inset-0 z-[999] bg-black/90 flex flex-col items-center justify-center" onClick={() => setShowRefModal(false)}>
          <button className="absolute top-5 right-10 text-white text-5xl z-[1001] cursor-pointer">&times;</button>
          <div className="relative w-full max-w-4xl h-[65vh] flex items-center justify-center" onClick={e => e.stopPropagation()}>
            <img src={refImages[currentIdx]} className="max-w-full max-h-full object-contain" alt="ref" />
            <button className="absolute left-0 text-white text-4xl p-4 bg-black/20" onClick={() => setCurrentIdx(currentIdx > 0 ? currentIdx - 1 : refImages.length - 1)}>&lt;</button>
            <button className="absolute right-0 text-white text-4xl p-4 bg-black/20" onClick={() => setCurrentIdx(currentIdx < refImages.length - 1 ? currentIdx + 1 : 0)}>&gt;</button>
          </div>
          <div className="flex gap-2 mt-8 overflow-x-auto max-w-full p-2" onClick={e => e.stopPropagation()}>
            {refImages.map((img, idx) => (
              <div key={idx} className={`w-20 h-20 flex-shrink-0 cursor-pointer border-2 ${currentIdx === idx ? 'border-blue-500' : 'border-transparent'}`} onClick={() => setCurrentIdx(idx)}>
